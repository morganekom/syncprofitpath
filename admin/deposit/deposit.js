...
async function handleAction(newStatus) {
    ...
    try {
        ...
        const userEmail = d.users?.email || '';
        const userName  = d.users?.full_name || d.users?.first_name || 'Investor';
        const fmtAmount = '$' + parseFloat(d.amount).toLocaleString('en-US', { minimumFractionDigits: 2 });

        if (userEmail) {
            await sendNotification({
                type:   newStatus === 'completed' ? 'deposit_approved' : 'deposit_rejected',
                email:  userEmail,
                name:   userName,
                amount: fmtAmount,
        ...
        // 4. Update local state so table reflects change without re-fetch
