...
async function loadPlans() {
    ...
    if (error) {
        ...
        grid.innerHTML = '<p style="color:var(--color-danger);">Failed to load plans. Please refresh.</p>';
        return;
    }

    if (!plans || plans.length === 0) {
        grid.innerHTML = '<p style="color:var(--color-gray-light);">No investment plans available.</p>';
        return;
    }
