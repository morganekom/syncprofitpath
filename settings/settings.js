...
function loadAllSavedData() {
    ...
    function applyPhoto(url) {
        ...
        if (url) {
            ...
            if (navPhoto) navPhoto.src = url;
        } else {
            // No photo set → blank avatar (like Facebook's default state)
            if (preview)  { preview.removeAttribute('src'); preview.style.display = 'none'; }
            if (navPhoto) { navPhoto.removeAttribute('src'); navPhoto.style.display = 'none'; }
        }
    }
